 ...
 * 
 */
public class StartPhotos extends BaseInsideAction {

  private static Logger log = Logger.getLogger(StartPhotos.class);

  public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    if(IS_DEBUG) log.debug("inside");

    //get photoAlbum coll from db
    PhotoService photoSvc = (PhotoService) getService(Constants.SERVICEKEY_PHOTO);
    Collection photoAlbumColl = photoSvc.findPhotoAlbumByUser(ActionUtils.getUserId(request));
    releaseService(Constants.SERVICEKEY_PHOTO, photoSvc);
    request.setAttribute("photoAlbumColl", photoAlbumColl);
